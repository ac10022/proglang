...
FrameSlot *add_slot(CodegenContext *codegen_context, IROperand operand) {
    ...
    codegen_context->next_offset -= operand.sym->typeinfo->size / 8;
...
int allocate_slots(CodegenContext *codegen_context, IRInstruction *ir_instruction) {
    int frame_size = (codegen_context->target == RV32 ? 8 : 16); // could be optimized (for example, if the function
                ...
                for (int i = 0; i < 3; i++) {
                    if (!find_slot(codegen_context, ir_operands[i])) {
                        if (is_variable(ir_operands[i]))
                            frame_size += add_slot(codegen_context, ir_instruction->dest)->operand.sym->typeinfo->size / 8;
                    }
                }
                break;
            case (IR_PARAM):
