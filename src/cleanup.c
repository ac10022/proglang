...
CleanupContext* new_cleanup_context(Arena* arena) {
    CleanupContext* cl_ctx = PALLOCT(arena, CleanupContext, 1);
    cl_ctx->notices = NULL;
    cl_ctx->arena = arena;
    return cl_ctx;
}
...
void append_new_notice(CleanupContext* cl_ctx, NoticeType type, bool requires_halt, const char* msg, ...) {
    ...
    char* fmt_msg = PALLOCS(cl_ctx->arena, len + 1);

    vsnprintf(fmt_msg, len + 1, msg, args_copy);
    va_end(args_copy);

    Notice* notice = PALLOCT(cl_ctx->arena, Notice, 1);
    notice->next = NULL;
    notice->type = type;
